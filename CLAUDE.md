...
Issues for this repo are stored as local markdown files under `.scratch/`. See `docs/agents/issue-tracker.md` for conventions and publishing/fetching rules.

### Triage labels

This repo uses the canonical triage vocabulary. The five roles are mapped to label strings in `docs/agents/triage-labels.md` (defaults: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`).

### Domain docs

This is a single-context repository: the primary domain file is `CONTEXT.md` at the repo root and ADRs live under `docs/adr/`. See `docs/agents/domain.md` for how skills should consume domain docs.
